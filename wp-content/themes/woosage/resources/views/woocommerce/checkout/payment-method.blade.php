...
<li class="wc_payment_method payment_method_{{ esc_attr( $gateway->id ) }}">
	<input id="payment_method_{{ esc_attr( $gateway->id ) }}" type="radio" class="input-radio" name="payment_method" value="{{ esc_attr( $gateway->id ) }}" @php checked( $gateway->chosen, true ) @endphp data-order_button_text="{{ esc_attr( $gateway->order_button_text ) }}" />
