				...
				<tr class="woocommerce-cart-form__cart-item {{ esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ) }}">
					...
					<td class="product-thumbnail">
						...
						@if ( ! $product_permalink ) {
							{!! $thumbnail; !!}
						@else
							@php printf( '<a href="%s">%s</a>', esc_url( $product_permalink ), $thumbnail ); // PHPCS: XSS ok. @endphp
						@endif
					</td>
		...
		<tr>
			<td colspan="6" class="actions">
				...
				@if ( wc_coupons_enabled() )
					<div class="coupon">
						<label for="coupon_code">{{ __( 'Coupon:', 'woocommerce' ) }}</label> <input type="text" name="coupon_code" class="input-text" id="coupon_code" value="" placeholder="{{ esc_attr( 'Coupon code', 'woocommerce' ) }}" /> <button type="submit" class="button" name="apply_coupon" value="{{ esc_attr( 'Apply coupon', 'woocommerce' ) }}">{{ esc_attr( 'Apply coupon', 'woocommerce' ) }}</button>
						@phpc do_action( 'woocommerce_cart_coupon' ); @endphp
					</div>
				@endif

				<button type="submit" class="button" name="update_cart" value="{{ esc_attr( 'Update cart', 'woocommerce' ) }}">{{ __( 'Update cart', 'woocommerce' ) }}</button>
